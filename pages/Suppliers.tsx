...
import { Plus, Edit2, Trash2, Building, MapPin, Globe, Phone, Mail, User, UserCheck, Truck, Tag, Wrench, Users, X, MessageCircle, Hash } from 'lucide-react';
...
export default function Suppliers() {
  ...
  const [formData, setFormData] = useState<Partial<Supplier>>({
    ...
    whatsapp: '', email: '', commercialContact: '', website: '', coordinates: undefined
  });
    ...
    <div className="animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <div>
            ...
            <p className="text-sm text-gray-500">Georreferencia, hoja de ruta y canales de comunicación.</p>
        </div>
        {isAdmin && (
                  ...
                  <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-slate-950 p-4 rounded-2xl flex-1 border border-gray-100 dark:border-slate-800">
                      ...
                      <div className="pt-2 space-y-3">
                          <div className="flex flex-wrap gap-2">
                              {supplier.whatsapp && (
                                  <a href={`https://wa.me/${supplier.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="flex items-center text-[10px] font-black uppercase text-white bg-green-600 px-3 py-1.5 rounded-full hover:bg-green-700 transition shadow-sm">
                                      <MessageCircle size={12} className="mr-1.5"/> WhatsApp
                                  </a>
                              )}
                              {supplier.email && (
                                  <a href={`mailto:${supplier.email}`} className="flex items-center text-[10px] font-black uppercase text-white bg-blue-600 px-3 py-1.5 rounded-full hover:bg-blue-700 transition shadow-sm">
                          ...
                          <div className="space-y-1">
                              {supplier.commercialContact && (
                                  <div className="flex items-center text-xs">
                                      ...
                                      <span className="font-bold text-gray-600 dark:text-gray-300 uppercase text-[10px] tracking-tight">{supplier.commercialContact}</span>
                                  </div>
                              )}
                              {supplier.address && (
                                  <div className="flex items-start text-[10px] text-gray-500 font-medium">
                                      <MapPin size={12} className="mr-2 text-red-400 flex-shrink-0 mt-0.5" />
                                      <span className="leading-tight">{supplier.address} {supplier.postalCode ? `(CP ${supplier.postalCode})` : ''}</span>
                                  </div>
                              )}
                          </div>
                      </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-gray-50 dark:bg-slate-950 p-6 rounded-[32px] border border-gray-100 dark:border-slate-800">
                        <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center">
                            <Building size={14} className="mr-2"/> Identidad Corporativa & Fiscal
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                ...
                                <input required type="text" className={inputClass} value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Ej: Semillas Global S.A." />
                            </div>
                            <div>
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1 mb-1.5">Sitio Web</label>
                                <input type="text" className={inputClass} value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} placeholder="https://..." />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-emerald-50/50 dark:bg-emerald-900/10 p-6 rounded-[32px] border border-emerald-100 dark:border-emerald-900/30">
                         <h3 className="text-[10px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-[0.2em] mb-4 flex items-center">
                            <Phone size={14} className="mr-2"/> Canales de Comunicación Directa
                         </h3>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1 mb-1.5">Persona de Contacto</label>
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1 mb-1.5">Email de Operaciones / Administración</label>
                                <div className="relative">
                                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-600" size={16}/>
                                    <input type="email" className={`${inputClass} pl-10`} value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="ventas@proveedor.com" />
                                </div>
                            </div>
                         </div>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="bg-blue-50/50 dark:bg-blue-900/10 p-6 rounded-[32px] border border-blue-100 dark:border-blue-900/30 flex flex-col h-full">
                        <h3 className="text-[10px] font-black text-blue-700 dark:text-blue-400 uppercase tracking-[0.2em] mb-4 flex items-center">
                            <MapPin size={14} className="mr-2"/> Localización Postal & GPS
                        </h3>
                        <div className="space-y-4 mb-4">
                            ...
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input type="text" placeholder="Dirección Postal" className={`${inputClass} pl-9`} value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                                </div>
                                <div className="relative w-24">
                                    <Hash className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14}/>
                                    <input type="text" placeholder="CP" className={`${inputClass} pl-9`} value={formData.postalCode} onChange={e => setFormData({...formData, postalCode: e.target.value})} />
                                </div>
                            </div>
                        </div>
                        <div className="flex-1 min-h-[200px] rounded-2xl overflow-hidden border dark:border-slate-800">
                             <MapEditor 
                        ...
                        <p className="text-[9px] font-black text-blue-400 uppercase mt-4 text-center tracking-tighter italic">Defina el punto exacto para cálculos de logística y fletes</p>
                    </div>
                </div>
              </div>
