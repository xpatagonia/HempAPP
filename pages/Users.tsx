...
export default function Users() {
  ...
  const handleQuickClientSubmit = async (e?: React.FormEvent) => {
      ...
      const newClient: Client = {
          ...
          contactName: formData.name || 'Titular Pendiente',
      ...
      if (success) {
          ...
          setQuickClientName('');
          alert(`Entidad "${newClient.name}" creada con éxito.`);
      }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (editingId) {
            ...
            if (!success) throw new Error("Fallo al actualizar en el servidor.");
        } else {
            if (!formData.password) { alert("Debes asignar una contraseña para el nuevo usuario."); setIsSaving(false); return; }
            const success = await addUser({ ...payload, id: Date.now().toString(), password: formData.password } as User);
            if (!success) throw new Error("Fallo al crear el usuario en el servidor.");
        }
        setIsModalOpen(false);
    } catch (err: any) {
        console.error("User submission error:", err);
        alert("Atención: El servidor no pudo procesar la solicitud. Verifique la pestaña SQL Cloud en Configuración.");
    ...
    <div className="animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 tracking-tight">Control de perfiles operativos y niveles de acceso.</p>
        </div>
        <button onClick={() => { setEditingId(null); setFormData({ name: '', email: '', role: 'technician', password: '', avatar: PRESET_AVATARS[0], clientId: '' }); setIsModalOpen(true); }} className="bg-hemp-600 text-white px-5 py-3 rounded-2xl flex items-center hover:bg-hemp-700 transition shadow-xl font-black text-xs uppercase tracking-widest">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border dark:border-slate-800 overflow-hidden overflow-x-auto">
        <table className="min-w-full text-sm text-left">
          <thead className="bg-gray-50 dark:bg-slate-950/50 text-gray-500 dark:text-slate-400 uppercase text-[10px] font-black tracking-widest border-b dark:border-slate-800">
            <tr>
              <th className="px-8 py-5 whitespace-nowrap">Usuario</th>
              <th className="px-8 py-5 whitespace-nowrap">Rol / Cargo</th>
              <th className="px-8 py-5 whitespace-nowrap">Entidad Asociada</th>
              <th className="px-8 py-5 text-right">Acciones</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
            {usersList.length === 0 ? (
                <tr><td colSpan={4} className="px-8 py-10 text-center text-gray-400 italic">No hay usuarios registrados.</td></tr>
            ) : usersList.map((user) => (
                ...
                <td className="px-8 py-5">
                   {(user.role === 'client' || user.role === 'technician') && user.clientId ? (
                       <div className="flex items-center text-gray-600 dark:text-gray-300 font-bold text-xs uppercase tracking-tight">
                           ...
                           {clients.find(c => c.id === user.clientId)?.name || 'Desconocida'}
                       </div>
                   ) : <span className="text-gray-300 dark:text-slate-600 italic text-xs">Administración Central</span>}
                </td>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl max-w-2xl w-full p-10 overflow-y-auto max-h-[95vh] animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-3xl font-black text-slate-800 dark:text-white uppercase tracking-tighter italic">Protocolo <span className="text-hemp-600">Usuario</span></h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-4">
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center"><UserIcon size={14} className="mr-2"/> Datos de Identidad</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 block mb-2">Nombre y Apellido *</label><input required className={inputClass} value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></div>
                        ...
                        <div><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 block mb-2">{editingId ? 'Nueva Contraseña' : 'Asignar Contraseña *'}</label><input type="text" className={inputClass} value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} placeholder={editingId ? 'Solo si desea cambiarla' : 'Mínimo 6 caracteres'}/></div>
                    </div>
                </div>

                <div className="space-y-4">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 block">Rango Jerárquico (Rol)</label>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-3xl border border-blue-100 dark:border-blue-900/30 animate-in slide-in-from-top-2">
                            ...
                            <div className="space-y-2 animate-in fade-in">
                                <div className="flex gap-2">
                                    ...
                                    <button 
                                        ...
                                        className="bg-blue-600 text-white px-4 rounded-2xl hover:bg-blue-700 transition shadow-md"
                                    >
                                        <Sparkles size={18}/>
                                    </button>
                                </div>
                                <p className="text-[9px] text-blue-500 font-bold uppercase ml-1">Crea la empresa primero para vincular este usuario.</p>
                            </div>
                        ) : (
                        ...
                        <p className="text-[9px] text-blue-400 mt-3 italic font-medium leading-relaxed">
                            {formData.role === 'client' 
                                ? 'El usuario "Productor" solo podrá gestionar recursos vinculados a esta entidad.' 
                                : 'Asignar al técnico a una entidad para segmentar reportes de campo.'}
                        </p>
                    </div>
                )}
                ...
                <div className="flex justify-end space-x-3 pt-8 border-t dark:border-slate-800 mt-4">
                    ...
                    <button type="submit" disabled={isSaving} className="bg-slate-900 dark:bg-hemp-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl flex items-center hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50">
                        ...
                        {editingId ? 'Actualizar Perfil' : 'Finalizar Alta'}
                    </button>
                </div>
            </form>
          </div>
