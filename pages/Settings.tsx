    ...
    <div className="max-w-4xl mx-auto pb-10 animate-in fade-in">
      <div className="flex items-center mb-6">
        ...
        <div>
            ...
            <p className="text-gray-500">Mantenimiento estructural y caché de red.</p>
        </div>
      </div>
      ...
      <div className="flex space-x-1 bg-gray-100 dark:bg-slate-900 p-1 rounded-lg mb-8 w-fit">
          ...
          <button onClick={() => setActiveTab('database')} className={`px-4 py-2 rounded-md text-sm font-black transition uppercase tracking-tighter ${activeTab === 'database' ? 'bg-white dark:bg-hemp-600 shadow text-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-700'}`}>SQL Nucleus V29</button>
      </div>
          ...
          <div className="space-y-6">
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-[40px] shadow-2xl relative overflow-hidden">
                  <div className="flex items-center space-x-3 mb-6">
                      ...
                      <h3 className="font-black text-white uppercase text-sm tracking-widest">Protocolo Nuclear V29</h3>
                  </div>
                  <div className="bg-amber-900/20 border border-amber-500/30 p-4 rounded-2xl mb-6 text-amber-200 text-xs">
                      ⚠️ IMPORTANTE: Ejecutar este script después de cualquier cambio manual en las tablas para limpiar la caché de la API (PGRST).
                  </div>
                  <button onClick={() => {
                      const sql = `
-- PROTOCOLO V29: REPARACIÓN TOTAL Y LIMPIEZA DE CACHÉ API
...
-- DESACTIVAR RLS PARA PRUEBAS (NUCLEAR)
...
-- FUEGO ATÓMICO: LIMPIAR CACHÉ DE ESQUEMA POSTGREST
NOTIFY pgrst, 'reload schema';
                      `;
                      navigator.clipboard.writeText(sql.trim());
                      alert("Script Nuclear V29 Copiado. Ejecútelo en Supabase SQL Editor para limpiar la caché del servidor.");
                      ...
                      <RefreshCw size={18} className="mr-2"/> Copiar Script de Limpieza V29
                  </button>
              </div>
          </div>
      )}
