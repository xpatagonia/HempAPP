  ...
  if (currentUser?.role !== 'super_admin') {
      return (
          <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4">
              ...
              <p className="text-gray-500">Solo el Super Administrador puede ver esta página.</p>
          </div>
      );
  }
    ...
    <div className="max-w-4xl mx-auto pb-10 animate-in fade-in">
      <div className="flex items-center mb-6">
        ...
        <div>
            ...
            <p className="text-gray-500">Mantenimiento estructural y limpieza de caché de red.</p>
        </div>
      </div>
      ...
      <div className="flex space-x-1 bg-gray-100 dark:bg-slate-900 p-1 rounded-lg mb-8 w-fit">
          ...
          <button onClick={() => setActiveTab('database')} className={`px-4 py-2 rounded-md text-sm font-black transition uppercase tracking-tighter ${activeTab === 'database' ? 'bg-white dark:bg-hemp-600 shadow text-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-700'}`}>SQL Nucleus V30</button>
      </div>
          ...
          <div className="space-y-6">
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-[40px] shadow-2xl relative overflow-hidden">
                  <div className="flex items-center space-x-3 mb-6">
                      ...
                      <h3 className="font-black text-white uppercase text-sm tracking-widest">Protocolo Nuclear V30</h3>
                  </div>
                  <div className="bg-red-900/20 border border-red-500/30 p-4 rounded-2xl mb-6 text-red-200 text-xs leading-relaxed">
                      ⚠️ ATENCIÓN: Este script soluciona el error "Schema Cache". <br/>
                      1. Cópielo con el botón verde. <br/>
                      2. Vaya a su panel de Supabase -> SQL Editor. <br/>
                      3. Pegue y ejecute (Run). <br/>
                      4. **IMPORTANTE:** Verifique que el mensaje final diga "Success" o "Query returned no rows".
                  </div>
                  <button onClick={() => {
                      const sql = `
-- PROTOCOLO V30: REPARACIÓN ESTRUCTURAL Y LIMPIEZA DE CACHÉ API POSTGREST
...
-- 2. Forzar tipos de datos correctos para arreglos (Causa común de Schema Cache Error)
DO $$ 
BEGIN
    -- Fix para locations
    IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='locations' AND COLUMN_NAME='responsible_ids') THEN
    ...
    -- Fix para projects
    IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='projects' AND COLUMN_NAME='responsible_ids') THEN
    ...
    -- Campos adicionales de locations
...
-- 3. Desactivar RLS para evitar bloqueos de permisos
...
-- 4. Otorgar permisos globales
GRANT ALL ON ALL TABLES IN SCHEMA public TO anon, authenticated;

-- 5. RECARGA NUCLEAR DE CACHÉ DE API (Solución definitiva al error Schema Cache)
NOTIFY pgrst, 'reload schema';
                      `;
                      navigator.clipboard.writeText(sql.trim());
                      alert("Script Nuclear V30 Copiado. Ejecútelo en Supabase SQL Editor para resetear la caché del servidor.");
                  }} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl flex items-center justify-center transition-all">
                      <RefreshCw size={18} className="mr-2"/> Copiar Script de Reparación V30
                  </button>
              </div>
          </div>
      )}
