...
import { TrialRecord, Plot, FieldLog, HydricRecord } from '../types';
import { 
  ...
  RefreshCw, Globe, Layers, Save, Thermometer, Droplets, Waves, QrCode, Printer, Loader2, Zap, Sparkles
} from 'lucide-react';
...
export default function PlotDetails() {
  ...
  // --- HUELLA HÍDRICA LOGIC (Precisión Industrial) ---
  useEffect(() => {
  ...
  const waterMetrics = useMemo(() => {
    // Registros manuales específicos de esta parcela o generales de la locación
    ...
    const totalLiters = totalMm * 10000 * areaHa; // 1mm en 1ha = 10.000 litros
    
    const latestYield = history.length > 0 ? history[0].yield : 0; // kg/ha
    const totalProductionKg = (latestYield || 800) * areaHa; // 800kg/ha es un default estimado
    
    return {
        totalMm,
        footprint: totalProductionKg > 0 ? Math.round(totalLiters / totalProductionKg) : 0,
        satellite: autoRain,
        manual: totalManual
    };
  ...
  const [recordForm, setRecordForm] = useState<Partial<TrialRecord>>({ 
    ...
    temperature: 0, humidity: 0, plantHeight: 0, plantsPerMeter: 0, replicate: 1, uniformity: 100, vigor: 100, lodging: 0, birdDamage: 0, yield: 0, stemWeight: 0, leafWeight: 0, freshWeight: 0
  });
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-8">
              <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-sm border dark:border-slate-800 overflow-hidden">
                  <div className="px-8 py-6 bg-gray-50 dark:bg-slate-950 border-b dark:border-slate-800 flex items-center"><ShieldCheck size={18} className="mr-2 text-hemp-600"/><h3 className="text-xs font-black uppercase tracking-[0.2em] dark:text-white">Certificación Lote</h3></div>
                  <div className="p-8 space-y-6">
                      {seedBatch ? (
                          <div className="space-y-4">
                              <div className="bg-gradient-to-br from-hemp-50 to-white dark:from-slate-800 dark:to-slate-900 p-6 rounded-3xl border border-hemp-100 dark:border-slate-700 text-center shadow-inner"><p className="text-[10px] font-black text-hemp-600 uppercase mb-2 tracking-widest">Lote Master</p><p className="text-2xl font-black text-hemp-900 dark:text-hemp-400 font-mono tracking-tighter">{seedBatch.batchCode}</p></div>
                          ...
                          <div className="text-center py-10"><AlertCircle size={32} className="mx-auto text-amber-500 mb-4 opacity-50"/><p className="text-[10px] text-gray-400 font-black uppercase tracking-widest leading-relaxed">Sin lote master vinculado.</p></div>
                      )}
                  </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              {activeTab === 'records' && (
                <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-sm border dark:border-slate-800 overflow-hidden animate-in fade-in">
                    <div className="px-10 py-6 border-b dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-950/50"><h2 className="font-black text-gray-900 dark:text-white uppercase text-[10px] tracking-[0.3em] flex items-center"><Activity size={18} className="mr-3 text-hemp-600"/> Historial de Medición</h2>{canEdit && <button onClick={() => { setIsRecordModalOpen(true); setIsViewMode(false); setEditingRecordId(null); }} className="bg-hemp-600 text-white px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-hemp-700 transition">Nuevo Registro</button>}</div>
              ...
              {activeTab === 'water' && (
                <div className="animate-in fade-in space-y-8">
                    <div className="bg-white dark:bg-slate-900 p-10 rounded-[40px] border dark:border-slate-800 shadow-sm relative overflow-hidden">
                         ...
                         <div className="mt-8 p-6 bg-blue-50 dark:bg-blue-900/10 rounded-3xl border border-blue-100 dark:border-blue-900/30 flex items-center justify-between">
                             <div>
                                 <p className="text-[10px] font-black text-blue-700 dark:text-blue-400 uppercase tracking-widest mb-1">Eficiencia de Recurso</p>
                                 <p className="text-sm font-bold text-blue-900 dark:text-blue-200">Este cultivo ha consumido <span className="font-black text-base">{waterMetrics.footprint} L</span> de agua por cada kilo de biomasa estimado.</p>
                             </div>
                             <Droplets size={32} className="text-blue-600 opacity-30"/>
                         </div>
                    </div>
                </div>
              )}
              ...
              {activeTab === 'logs' && (
                <div className="space-y-4 animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 rounded-[40px] p-10 border dark:border-slate-800 shadow-sm">
                        <div className="flex justify-between items-center mb-10"><h2 className="font-black text-gray-900 dark:text-white uppercase text-[10px] tracking-[0.4em] flex items-center"><MessageSquare size={18} className="mr-3 text-blue-500"/> Bitácora Digital Multimedia</h2><button onClick={() => setIsLogModalOpen(true)} className="bg-blue-600 text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-blue-700 transition flex items-center"><Camera size={16} className="mr-2"/> Captura de Lote</button></div>
      ...
      {/* MODAL RECORD (Same structure as previous version but ensures logic) */}
      {isRecordModalOpen && (
          // El contenido del modal se mantiene pero se asegura la conversión numérica en handleSaveRecord definida arriba
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
              {/* ... (contenido del modal omitido por brevedad pero integrado en la lógica superior) ... */}
              <div className="bg-white dark:bg-slate-900 rounded-[40px] max-w-4xl w-full p-10 shadow-2xl max-h-[95vh] overflow-y-auto">
                   <div className="flex justify-between items-center mb-8">
                       <h2 className="text-2xl font-black dark:text-white uppercase italic tracking-tighter">Monitoreo <span className="text-hemp-600">Fenológico</span></h2>
                       <button onClick={() => setIsRecordModalOpen(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full transition"><X size={28} className="text-gray-400"/></button>
                   </div>
                   <form onSubmit={handleSaveRecord} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="space-y-4">
                                <label className={labelClass}>Cronología</label>
                                <input type="date" required className={inputStyle} value={recordForm.date} onChange={e => setRecordForm({...recordForm, date: e.target.value})} />
                                <input type="time" required className={inputStyle} value={recordForm.time} onChange={e => setRecordForm({...recordForm, time: e.target.value})} />
                                <select required className={inputStyle} value={recordForm.stage} onChange={e => setRecordForm({...recordForm, stage: e.target.value as any})}>
                            ...
                            <div className="md:col-span-2 grid grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-950 p-6 rounded-3xl">
                                <div><label className={labelClass}>Altura (cm)</label><input type="number" step="0.1" className={inputStyle} value={recordForm.plantHeight} onChange={e => setRecordForm({...recordForm, plantHeight: Number(e.target.value)})} /></div>
                                <div><label className={labelClass}>Rinde Est. (kg/ha)</label><input type="number" className={`${inputStyle} border-blue-200`} value={recordForm.yield} onChange={e => setRecordForm({...recordForm, yield: Number(e.target.value)})} /></div>
                                <div><label className={labelClass}>Vigor (1-100)</label><input type="number" className={inputStyle} value={recordForm.vigor} onChange={e => setRecordForm({...recordForm, vigor: Number(e.target.value)})} /></div>
                                <div><label className={labelClass}>Pájaros (%)</label><input type="number" className={inputStyle} value={recordForm.birdDamage} onChange={e => setRecordForm({...recordForm, birdDamage: Number(e.target.value)})} /></div>
                            </div>
                        </div>
                        <button type="submit" disabled={isSaving} className="w-full bg-hemp-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-xl flex items-center justify-center">
      ...
      {isLogModalOpen && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
              <div className="bg-white dark:bg-slate-900 rounded-[40px] max-w-lg w-full p-10 shadow-2xl">
                   <h2 className="text-2xl font-black dark:text-white uppercase italic mb-8">Nota <span className="text-blue-600">Multimedia</span></h2>
                   <form onSubmit={handleSaveLog} className="space-y-6">
                        <textarea required className={`${inputStyle} h-40`} value={logForm.note} onChange={e => setLogForm({...logForm, note: e.target.value})} placeholder="Detalles de la observación en campo..."></textarea>
                        <button type="submit" disabled={isSaving} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-xl">Publicar</button>
                   </form>
              </div>
          </div>
      )}
