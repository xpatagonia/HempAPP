...
export default function PlotDetails() {
  ...
  const handleSaveMap = async () => {
      setIsSaving(true);
      try {
          updatePlot({ ...plot, polygon: tempPolygon, surfaceArea: tempArea, coordinates: tempCoords });
                  ...
                  <div className="p-8 space-y-6">
                      {seedBatch ? (
                          <div className="space-y-4">
                              <div className="bg-gradient-to-br from-hemp-50 to-white p-4 rounded-2xl border border-hemp-100 text-center"><p className="text-[10px] font-black text-hemp-600 uppercase mb-1 tracking-widest">Lote Master</p><p className="text-xl font-black text-hemp-900 font-mono">{seedBatch.batchCode}</p></div>
                              <div className="space-y-3 text-xs"><div className="flex justify-between border-b border-dashed pb-2 font-bold text-gray-500"><span>Categoría:</span><span className="text-gray-800">{seedBatch.category}</span></div><div className="flex justify-between border-b border-dashed pb-2 font-bold text-gray-500"><span>Etiqueta:</span><span className="text-gray-800 font-mono">{seedBatch.labelSerialNumber || '-'}</span></div><div className="flex justify-between pb-2 font-bold text-gray-500"><span>Certificación:</span><span className="text-blue-600 truncate">{seedBatch.certificationNumber || 'N/A'}</span></div></div>
                          </div>
                      ) : (
                          <div className="text-center py-6"><AlertCircle size={32} className="mx-auto text-amber-500 mb-2 opacity-50"/><p className="text-[10px] text-gray-400 font-black uppercase tracking-widest leading-relaxed">Sin lote master vinculado.<br/>Vincular en ajustes del lote.</p></div>
                      )}
                  </div>
              </div>

              <div className="bg-white rounded-3xl shadow-sm border overflow-hidden">
                  <div className="px-6 py-5 bg-gray-50 border-b flex items-center justify-between"><div className="flex items-center"><Globe size={18} className="mr-2 text-blue-600"/><h3 className="text-xs font-black uppercase tracking-[0.2em]">Diseño de Unidad</h3></div>{canEdit && <button onClick={() => setIsMapModalOpen(true)} className="p-1.5 hover:bg-blue-100 text-blue-600 rounded-lg transition"><Maximize2 size={16}/></button>}</div>
                  <div className="h-56 relative group">
                      {plot.polygon && plot.polygon.length > 2 ? <MapEditor initialCenter={plot.coordinates} initialPolygon={plot.polygon} readOnly={true} height="100%" /> : <div className="h-full bg-slate-50 flex flex-col items-center justify-center text-gray-400 border-b"><Map size={32} className="mb-2 opacity-30"/><p className="text-[10px] font-black uppercase tracking-widest">Sin delimitación GPS</p>{canEdit && <button onClick={() => setIsMapModalOpen(true)} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Delimitar Polígono</button>}</div>}
                  </div>
                  <div className="p-6 bg-white grid grid-cols-2 gap-4"><div className="text-center border-r"><p className="text-[9px] font-black text-gray-400 uppercase mb-1">Superficie Real</p><p className="text-sm font-black text-gray-800">{plot.surfaceArea} {plot.surfaceUnit}</p></div><div className="text-center"><p className="text-[9px] font-black text-gray-400 uppercase mb-1">Área en M²</p><p className="text-sm font-black text-gray-800">{plot.surfaceUnit === 'ha' ? (plot.surfaceArea || 0) * 10000 : plot.surfaceArea} m²</p></div></div>
              </div>
          </div>

          <div className="lg:col-span-2 space-y-6">
              <div className="flex bg-white p-1.5 rounded-2xl border shadow-sm w-fit overflow-x-auto">
                  ...
                  <button onClick={() => setActiveTab('water')} className={`px-6 py-2.5 font-black text-[10px] uppercase tracking-widest rounded-xl transition-all whitespace-nowrap ${activeTab === 'water' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-600'}`}>Balance Hídrico</button>
              ...
              {activeTab === 'records' && (
                <div className="bg-white rounded-3xl shadow-sm border overflow-hidden animate-in fade-in slide-in-from-top-2">
                    <div className="px-8 py-5 border-b flex justify-between items-center bg-gray-50/50"><h2 className="font-black text-gray-900 uppercase text-[10px] tracking-[0.2em] flex items-center"><Activity size={16} className="mr-2 text-hemp-600"/> Historial de Monitoreo</h2>{canEdit && <button onClick={handleOpenNewRecord} className="bg-hemp-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg hover:bg-hemp-700 transition">Nuevo Registro</button>}</div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm text-left">
                            <thead className="bg-gray-50/50 text-gray-400 uppercase text-[9px] font-black tracking-widest"><tr><th className="px-8 py-4">Fecha/Hora</th><th className="px-8 py-4">Etapa</th><th className="px-8 py-4 text-center">Altura</th><th className="px-8 py-4 text-center">Clima</th><th className="px-8 py-4 text-right">Ver</th></tr></thead>
                            <tbody className="divide-y divide-gray-100">{history.length === 0 ? ( <tr><td colSpan={5} className="p-12 text-center text-gray-300 italic font-medium">No hay monitoreos registrados.</td></tr> ) : history.map(r => ( <tr key={r.id} className="hover:bg-gray-50 cursor-pointer group" onClick={() => { setEditingRecordId(r.id); setRecordForm(r); setIsViewMode(true); setIsRecordModalOpen(true); }}><td className="px-8 py-5 font-black text-gray-800">{r.date} <span className="block text-[10px] text-gray-400 font-normal">{r.time || '--:--'}</span></td><td className="px-8 py-5"><span className="px-3 py-1 rounded-full text-[9px] font-black uppercase border bg-green-50 text-green-700 border-green-100">{r.stage}</span></td><td className="px-8 py-5 font-black text-gray-900 text-center">{r.plantHeight ? `${r.plantHeight} cm` : '-'}</td><td className="px-8 py-5 text-center">{r.temperature !== undefined ? ( <div className="flex flex-col items-center"><span className="text-xs font-bold text-orange-600">{r.temperature}°C</span><span className="text-[10px] text-blue-500 font-medium">{r.humidity}% HR</span></div> ) : '-'}</td><td className="px-8 py-5 text-right"><Eye size={18} className="text-gray-300 group-hover:text-hemp-600 ml-auto transition"/></td></tr> ))}</tbody>
                        </table>
                    </div>
                </div>
              )}
              ...
              {activeTab === 'logs' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                    <div className="bg-white rounded-3xl p-8 border shadow-sm">
                        <div className="flex justify-between items-center mb-8"><h2 className="font-black text-gray-900 uppercase text-[10px] tracking-[0.2em] flex items-center"><MessageSquare size={16} className="mr-2 text-blue-500"/> Notas de Campo Multimedia</h2><button onClick={() => setIsLogModalOpen(true)} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg hover:bg-blue-700 transition flex items-center"><Camera size={16} className="mr-2"/> Agregar Nota con Foto</button></div>
                        <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-blue-200 before:via-blue-100 before:to-transparent">{plotLogs.length === 0 ? <div className="text-center py-12 text-gray-400 italic bg-gray-50 rounded-2xl border border-dashed">No hay entradas en la bitácora.</div> : plotLogs.map(log => ( <div key={log.id} className="relative flex items-start group"><div className="absolute left-0 w-10 h-10 bg-white border-4 border-blue-50 rounded-full flex items-center justify-center z-10 shadow-sm group-hover:scale-110 transition-transform"><div className="w-2.5 h-2.5 rounded-full bg-blue-600"></div></div><div className="ml-14 flex-1 bg-gray-50/50 rounded-2xl p-6 border border-gray-100 hover:border-blue-200 hover:bg-white transition-all shadow-sm"><div className="flex justify-between items-start mb-4"><div className="flex flex-col"><div className="flex items-center"><Calendar size={12} className="text-blue-500 mr-2"/><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{log.date}</span></div><div className="flex items-center mt-0.5 opacity-60"><Clock size={10} className="text-gray-400 mr-2"/><span className="text-[10px] font-bold text-gray-500">{log.time || '--:--'}</span></div></div>{isAdmin && <button onClick={() => window.confirm("¿Borrar?") && deleteLog(log.id)} className="p-1 hover:bg-red-50 text-gray-300 hover:text-red-500 rounded transition opacity-0 group-hover:opacity-100"><Trash2 size={14}/></button>}</div><p className="text-sm text-gray-700 leading-relaxed font-medium mb-5">{log.note}</p>{log.photoUrl && <div className="relative rounded-2xl overflow-hidden border border-gray-200 max-w-sm cursor-zoom-in" onClick={() => setPreviewPhoto(log.photoUrl!)}><img src={log.photoUrl} className="w-full h-auto object-cover" /></div>}</div></div> ))}</div>
                    </div>
                </div>
              )}
          </div>
